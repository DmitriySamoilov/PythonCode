...
#v1
print ('Парам пам-пам' if len(set((map( lambda x: len([i for i in x if i in 'ауоеияюэы' ]),(st.lower()).split(' '))))) == 1 else 'Пам парам')

#v2 c filter()
print ('Парам пам-пам'if len(set((map(lambda x: len(list(filter ( lambda i: i in 'ауоеияюэы' , x  ))), (st.lower()).split(' '))))) == 1 else 'Пам парам')
